...
FeedForwardNetwork<T, D_I, D_O, D_H>::FeedForwardNetwork() {
...
template<typename T, int D_I, int D_O, int D_H>
void FeedForwardNetwork<T, D_I, D_O, D_H>::load_params(FeedForwardNetworkParam<T, D_I, D_O, D_H> *p) {
    if(p != nullptr) {
...
template<typename T, int D_I, int D_O, int D_H>
void FeedForwardNetwork<T, D_I, D_O, D_H>::forward(T input[D_I], T output[D_O]) {
    T tmp[3][D_H];
