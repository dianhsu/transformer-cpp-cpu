...
Decoder<T, DIM, DEP, D_H, HEAD_SIZE, LAYER_CNT>::Decoder() {
...
template<typename T, int DIM, int DEP, int D_H, int HEAD_SIZE, int LAYER_CNT>
void Decoder<T, DIM, DEP, D_H, HEAD_SIZE, LAYER_CNT>::load_params(DecoderParam<T, DIM, D_H, HEAD_SIZE, LAYER_CNT> *p) {
    if(p != nullptr) {
        for(int i = 0; i < LAYER_CNT; ++i) {
...
template<typename T, int DIM, int DEP, int D_H, int HEAD_SIZE, int LAYER_CNT>
void Decoder<T, DIM, DEP, D_H, HEAD_SIZE, LAYER_CNT>::forward(T input[DEP][DIM], T output[DEP][DIM]) {
